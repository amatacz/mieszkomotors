...
from mieszkomotors.models.events import SpringTyresReplacementEvent, \
    SpringTyresReplacementEventMail
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        '''
        Command to create mail objects for each Spring Tyres Replacement Event
        (object assigned to each car).
        Method runs once for a year, March 15th and creates
        SpringTyresReplacementEventMail with car and car owner data.
        '''
        current_year = date.today().year

        spring_tyres_replacement_events = SpringTyresReplacementEvent. \
            objects.filter(start__year=current_year)

        if spring_tyres_replacement_events:
            for spring_tyres_replacement_event in \
                    spring_tyres_replacement_events:
                owner = CarOwner.objects.filter(
                    car=spring_tyres_replacement_event.car.pk).filter(
                    status='a')[0]
                ...
                if hasattr(owner.client, 'individual_customer'):
                    ...
                    SpringTyresReplacementEventMail.objects.create(
                        spring_tyres_replacement_event=(
                            spring_tyres_replacement_event),
