...
'''
Command to create mail objects for each Winter Tyres Replacement Event (object assigned to each car).
Method runs once for a year, November 15th and creates WinterTyresReplacementEventMail with car and car owner data.
'''
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        winter_tyres_replacement_events = WinterTyresReplacementEvent.objects.filter(start__year = current_year)

        if winter_tyres_replacement_events:
            for winter_tyres_replacement_event in winter_tyres_replacement_events:
                owner = CarOwner.objects.filter(car=winter_tyres_replacement_event.car.pk).filter(status = 'a')[0]
                ...
                if hasattr(owner.client, 'individual_customer'):
                    ...
                    WinterTyresReplacementEventMail.objects.create(
                        winter_tyres_replacement_event = winter_tyres_replacement_event,
