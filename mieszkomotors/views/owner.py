...
class IndividualCustomerDetail(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['individual_customers'] = IndividualCustomer.objects.filter(
            id=self.kwargs['pk']
            )
        context['attachments'] = CustomerAttachment.objects.filter(
            customer__individual_customer=self.kwargs['pk']
            )
        context['notes'] = CustomerNote.objects.filter(
            customer__individual_customer=self.kwargs['pk']
            )
        return context
...
class IndividualCustomerUpdate(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy('individual_customer_detail',
...
class SelfEmployedCustomerDetail(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['self_employed_customers'] = \
            SelfEmployedCustomer.objects.filter(id=self.kwargs['pk'])
        context['attachments'] = CustomerAttachment.objects.filter(
            customer__self_employed_customer=self.kwargs['pk']
            )
        context['notes'] = CustomerNote.objects.filter(
            customer__self_employed_customer=self.kwargs['pk']
            )
...
class EnterpriseCustomerDetail(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['enterprise_customers'] = EnterpriseCustomer.objects.filter(
            id=self.kwargs['pk']
            )
        context['attachments'] = CustomerAttachment.objects.filter(
            customer__enterprise_customer=self.kwargs['pk']
            )
        context['notes'] = CustomerNote.objects.filter(
            customer__enterprise_customer=self.kwargs['pk']
            )
...
class CustomerNoteCreate(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy(f'{self.get_name()}_customer_detail',
                            args=[self.kwargs['customer_id']])

    def get_customer(self):
        if self.kwargs['customer_type'] == 'individual':
            return IndividualCustomer.objects.get(
                id=self.kwargs['customer_id'])
        if self.kwargs['customer_type'] == 'selfemployed':
            return SelfEmployedCustomer.objects.get(
                id=self.kwargs['customer_id'])
        if self.kwargs['customer_type'] == 'enterprise':
