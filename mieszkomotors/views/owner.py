...
class OwnersList(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['individual_owners'] = IndividualOwner.objects.all()
        context['self_employed_owners'] = SelfEmployedOwner.objects.all()
        context['enterprise_owners'] = EnterpriseOwner.objects.all()
        return context
...
class IndividualOwnerDetail(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['individual_owners'] = IndividualOwner.objects.all().filter(id = self.kwargs['pk'])
        context['attachments'] = IndividualOwnerAttachment.objects.all().filter(owner = self.kwargs['pk'])
        context['notes'] = IndividualOwnerNotes.objects.all().filter(owner = self.kwargs['pk'])
        return context
