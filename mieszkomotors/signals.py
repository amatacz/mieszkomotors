...
from mieszkomotors.models.events import CarEvent, InsuranceEvent, \
    SpringTyresReplacementEvent, WinterTyresReplacementEvent
...
from mieszkomotors.models.owner import IndividualCustomer,\
    SelfEmployedCustomer, EnterpriseCustomer, Customer
...
def set_car_review_dates(sender, instance, created, **kwargs):
    if created:
        instance.car_review_renewal_date = instance.current_car_review_date \
...
def change_car_review_dates(sender, instance, **kwargs):
    try:
        ...
        if updated_car.current_car_review_date != \
                instance.current_car_review_date:
            instance.car_review_renewal_date = \
                    instance.current_car_review_date \
                    + relativedelta(years=+1)
            updated_car.save()
...
def set_insurance_date(sender, instance, created, **kwargs):
    if created:
        instance.insurance_renewal_date = instance.current_insurance_date \
            + relativedelta(years=+1)
        instance.save()
...
def change_insurance_dates(sender, instance, **kwargs):
    try:
        ...
        if updated_insurance.current_insurance_date !=  \
                instance.current_insurance_date:
            instance.insurance_renewal_date = instance.current_insurance_date \
                + relativedelta(years=+1)
...
# Assignes insurance partial payments as FK to Insurance object
# when Insurance Partial Payments object is created
...
def create_car_event(sender, instance, created, **kwargs):
    if created:
        ...
        car_event = CarEvent.objects.create(car=instance, title=title,
                                            start=start)
        car_event.title = f'Przegląd techniczny {car_event.car.license_plates}'
        car_event.description = f'{RENEWAL_INTERVAL} dni do odnowienia \
            przeglądu {car_event.car.license_plates}. \
            Skontaktu się z właścicielem samochodu.'
        car_event.start = car_event.car.car_review_renewal_date \
            - relativedelta(days=RENEWAL_INTERVAL)
...
def create_insurance_event(sender, instance, created, **kwargs):
    if created:
        ...
        insurance_event = InsuranceEvent.objects.create(insurance=instance,
                                                        title=title,
                                                        start=start)
        insurance_event.title = f'Odnowienie ubezpieczenia \
            {insurance_event.insurance.car.license_plates}'
        insurance_event.description = f'{RENEWAL_INTERVAL} dni do odnowienia \
            ubezpieczenia {insurance_event.insurance.car.license_plates}. \
                Skontaktuj się z właścicielem samochodu.'
        insurance_event.start = \
            insurance_event.insurance.insurance_renewal_date \
            - relativedelta(days=RENEWAL_INTERVAL)
...
def change_insurance_event_dates(sender, instance, **kwargs):
    try:
        updated_insurance_event = InsuranceEvent.objects.get(
            insurance=instance.id)
        if updated_insurance_event.insurance.current_insurance_date != \
                instance.current_insurance_date:
            updated_insurance_event.start = instance.insurance_renewal_date \
                - relativedelta(days=RENEWAL_INTERVAL)
...
def change_car_event_dates(sender, instance, **kwargs):
    try:
        ...
        if updated_car_event.car.current_car_review_date != \
                instance.current_car_review_date:
            updated_car_event.start = instance.car_review_renewal_date \
                - relativedelta(days=RENEWAL_INTERVAL)
...
def create_spring_tyres_replacement_event(sender, instance, created, **kwargs):
    if created:
        ...
        spring_tyres_replacement_event = SpringTyresReplacementEvent.objects. \
            create(car=instance, title=title, start=start)
        spring_tyres_replacement_event.title = "Zmiana opon na letnie"
        spring_tyres_replacement_event.description = "Przypominam o zmianie \
            opon na letnie"
...
def create_winter_tyres_replacement_event(sender, instance, created, **kwargs):
    if created:
        ...
        winter_tyres_replacement_event = WinterTyresReplacementEvent.objects. \
            create(car=instance, title=title, start=start)
        winter_tyres_replacement_event.title = "Zmiana opon na zimowe"
        winter_tyres_replacement_event.description = "Przypominam o zmianie \
            opon na zimowe"
...
def create_customer_when_individual_owner_is_created(sender, created,
                                                     instance, **kwargs):
    if created:
...
def create_customer_when_self_employed_owner_is_created(sender, created,
                                                        instance, **kwargs):
    if created:
...
def create_customer_when_enterprise_owner_is_created(sender, created,
                                                     instance, **kwargs):
    if created:
        customer = Customer.objects.create(identifier=instance.regon)
        instance.customer = customer
...
def post_delete_self_employed_customer(sender, instance, *args, **kwargs):
...
def post_delete_enterprise_customer(sender, instance, *args, **kwargs):
    customer = Customer.objects.get(identifier=instance.regon)
    customer.delete()
