...
from mieszkomotors.models.owner import Customer, IndividualCustomer, \
    SelfEmployedCustomer, EnterpriseCustomer

format = "%Y-%m-%d %H:%M:%S"


class CarEventsSerializer(serializers.ModelSerializer):
    publication_datetime = serializers.DateTimeField(format=format)
    update_datetime = serializers.DateTimeField(format=format)
