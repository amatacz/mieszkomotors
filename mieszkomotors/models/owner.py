...
class Customer(PublicationTracker):
    ...
    def __str__(self):
        try:
            ...
            return f'{individual_owner.first_name} \
        {individual_owner.last_name}'
        except IndividualCustomer.DoesNotExist:
            try:
                self_employed_owner = SelfEmployedCustomer.objects.get(
                    customer=self.pk)
                return f'{self_employed_owner.first_name} \
                    {self_employed_owner.last_name}'
            except SelfEmployedCustomer.DoesNotExist:
                try:
                    enterprise_owner = EnterpriseCustomer.objects.get(
                        customer=self.pk)
...
class ContactData(PublicationTracker):
    ...
    address_prefix = models.CharField(max_length=3, choices=ADDRES_PREFIXES,
                                      default="ul.")
...
class IndividualCustomer(ContactData):
    ...
    customer = models.OneToOneField(Customer, on_delete=models.CASCADE,
                                    related_name='individual_customer',
                                    null=True, blank=True)
    first_name = models.CharField(max_length=30)
    last_name = models.CharField(max_length=50)
    pesel = models.CharField(max_length=11, unique=True,
                             verbose_name="Identyfikator - PESEL")
...
class SelfEmployedCustomer(ContactData):
    ...
    customer = models.OneToOneField(Customer, on_delete=models.CASCADE,
                                    related_name='self_employed_customer',
                                    null=True, blank=True)
    ...
    regon = models.CharField(max_length=11, unique=True,
                             verbose_name="Identyfikator - REGON")
    driving_license_since = models.DateField(default='1970-01-01',
                                             null=True, blank=True)
    customer_type = models.CharField(max_length=15, choices=CUSTOMER_TYPE,
...
class EnterpriseCustomer(ContactData):
    ...
    customer = models.OneToOneField(Customer, on_delete=models.CASCADE,
                                    related_name='enterprise_customer',
                                    null=True, blank=True)
